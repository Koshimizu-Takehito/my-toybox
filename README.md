...
Each `"id"` in `Screens.json` is used directly as an `enum Screen` case name, so it **must be a valid Swift identifier in lowerCamelCase (e.g. `gameOfLifeScreen`)**.
...
| Plugin | Input | Output |
|--------|-------|--------|
| `GenerateScreenID` | `Screens.json` | `Screen.swift` (enum cases for each screen) |
...
my-toybox/
  ...
  │   │   └─ GenerateScreenID/     # SPM plugin for Screen generation
  │   ├─ Sources/
  ...
  └─ Scripts/
      ...
      ├─ generate_screen_id.sh     # Screen generator (used by plugin)
...
> **Note**: Metal shaders and `Screen.swift` are automatically generated by SPM plugins during build.
