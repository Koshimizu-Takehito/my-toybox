...
namespace VoronoiDiagramShadeder2 {
    float length3(float2 x, float time) {
        float ratio = fract(time);
        float len1 = Metric::length2(x, Metric::Functions(int(time) % 3));
        float len2 = Metric::length2(x, Metric::Functions(int(time + 1) % 3));
        return mix(len1, len2, smoothstep(0, 1, ratio));
    }

    // 距離関数を入れ替えたボロノイ胞体
    float2 voronoi_custom_22(float2 p, float time) {
        // ボロノイ胞体の ID 変数
        float2 id;
        // 最も近い格子点
        float2 n = floor(p + 0.5);
        ...
        for(float j = 0.0; j <= 2.0; j ++ ) {
            ...
            for(float i = -1.0; i <= 1.0; i ++ ) {
                glid.x = n.x + i;
                float2 jitter = (Hash::hash22(glid) - 0.5);
                float2 x = glid + jitter - p;
                if (length3(x, time) <= dist) {
                    id = glid;
                }
                dist = min(dist, length3(x, time));
            }
        }
        return id;
    }
    ...
    /// ボロノイ胞体
    [[ stitchable ]] half4 main(float2 position, half4 color, float4 box, float secs) {
        float2 pos = 5 * position / min(box.z, box.w);
        float2 voronoi = voronoi_custom_22(pos, secs * __FLT_M_PI__ / 4);
        float hash = Hash::hash21(voronoi);
        half3 hsv = half3(hash, (0.5 + hash/2), 1.0);
        half3 rgb = Color::hsv2rgb(hsv);
        return half4(rgb, 1.0);
    }
}
